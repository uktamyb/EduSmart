...
interface Group { id: string; name: string; subjectName: string | null }
...
interface HistoryRecord { date: string; avg: number; count: number }
...
export default function TeacherGrades() {
  ...
  async function fetchStudentsAndGrades() {
    ...
    ;(gradeData ?? []).forEach((r) => { map[r.student_id] = { grade: String(r.grade), comment: r.comment ?? '' } })
    setGradeMap(map)
  ...
  async function fetchHistory() {
    ...
    if (data) {
      ...
      setHistory(Object.entries(byDate).map(([d, grades]) => ({
        date: d,
        count: grades.length,
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const { error } = await supabase.from('grades').upsert(records, { onConflict: 'student_id,group_id,date' })
    if (!error) {
      setSaved(true)
      ...
      for (const s of students) {
        ...
        sendTelegramMessage(s.parent_telegram, `Hurmatli ${parentName}, ${s.full_name} ning bahosi: ${entry.grade}/100. ${group?.subjectName ?? ''} ${date}`)
      }
    }
    setSaving(false)
  }
  ...
  const gradeValues = Object.values(gradeMap).map((e) => parseInt(e.grade)).filter((n) => !isNaN(n))
  const avg = gradeValues.length > 0 ? Math.round(gradeValues.reduce((s, g) => s + g, 0) / gradeValues.length) : null
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setSaved(false) }} className="border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {[
          { color: 'green', label: '≥86 — A\'lo' },
          { color: 'blue', label: '≥71 — Yaxshi' },
          { color: 'yellow', label: '≥56 — Qoniqarli' },
          { color: 'red', label: '<56 — Qoniqarsiz' },
        ].map((item) => (
          <span key={item.label} className={`inline-flex items-center gap-1.5 text-xs font-medium text-${item.color}-700 bg-${item.color}-50 border border-${item.color}-200 px-3 py-1 rounded-full`}>
            <span className={`w-2 h-2 rounded-full bg-${item.color}-500 inline-block`} />
            {item.label}
          </span>
        ))}
      </div>
      ...
      {!selectedGroupId ? (
        ...
        students.length === 0 ? <p className="text-gray-400">Bu guruhda o'quvchi yo'q.</p> : (
          <div>
            <div className="flex gap-4 mb-4">
              <span className="text-sm text-gray-600 font-medium">Baholandi: {gradedCount}/{students.length}</span>
              {avg !== null && <span className="text-sm text-blue-600 font-medium">O'rtacha: {avg}</span>}
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-[520px]">
                  ...
                  <tbody className="divide-y">
                    {students.map((s, i) => {
                      const entry = gradeMap[s.id] ?? { grade: '', comment: '' }
                      const gradeNum = parseInt(entry.grade)
                      const color = entry.grade === '' ? 'text-gray-300' : gradeNum >= 86 ? 'text-green-600' : gradeNum >= 71 ? 'text-blue-600' : gradeNum >= 56 ? 'text-yellow-600' : 'text-red-500'
                      return (
                        <tr key={s.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 text-gray-400">{i + 1}</td>
                          <td className="px-4 py-3 text-gray-800 font-medium">{s.full_name}</td>
                          <td className="px-4 py-3">
                            <input type="number" min="0" max="100" value={entry.grade} onChange={(e) => setGrade(s.id, e.target.value)} placeholder="—" className={`w-20 border rounded px-2 py-1 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 ${color}`} />
                          </td>
                          <td className="px-4 py-3">
                            <input type="text" value={entry.comment} onChange={(e) => setComment(s.id, e.target.value)} placeholder="Ixtiyoriy..." className="w-full border rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                          </td>
                        </tr>
                      )
                    })}
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleSave} disabled={saving || gradedCount === 0} className="bg-blue-600 text-white px-6 py-2 rounded text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
        ...
        history.length === 0 ? <p className="text-gray-400">Baholar tarixi yo'q.</p> : (
                  ...
                  {history.map((row) => (
                    <tr key={row.date} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`font-semibold ${row.avg >= 86 ? 'text-green-600' : row.avg >= 71 ? 'text-blue-600' : row.avg >= 56 ? 'text-yellow-600' : 'text-red-500'}`}>
                          {row.avg}
                        </span>
                      </td>
                    </tr>
