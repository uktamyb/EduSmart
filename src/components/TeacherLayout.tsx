...
const navItems = [
  ...
  { to: '/teacher/attendance', label: 'Davomat' },
  { to: '/teacher/grades', label: 'Baholar' },
]
...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-100">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-56 bg-gray-900 text-gray-100 flex flex-col transition-transform duration-200
      ...
      >
        <div className="px-6 py-5 border-b border-gray-700 flex items-center justify-between">
          <div>
            ...
            <span className="block text-xs text-blue-400 font-medium truncate max-w-[120px]">
              {fullName ?? 'O\'qituvchi'}
            </span>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="md:hidden bg-gray-900 text-white px-4 py-3 flex items-center gap-3 shrink-0">
          <button onClick={() => setSidebarOpen(true)} className="p-1">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </button>
          <span className="font-semibold">{fullName ?? "O'qituvchi"}</span>
        </div>
